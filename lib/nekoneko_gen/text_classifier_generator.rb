...
module NekonekoGen
  class TextClassifierGenerator
    ...
    def train(iteration = 30)
      ...
      under_sample = data.map{|v| v.size}.min
      ...
      iteration.times do |step|
        ...
        @arow.k.times.map do |i|
          sampling(data[i], under_sample).map {|vec| [vec, i] }
        end.flatten(1).shuffle!.each do |v|
          loss += @arow.update(v[0], v[1])
          c += 1
        end
        print sprintf(" ER: %.6f, %.4fs\n", loss / c.to_f, Time.now - t)
      end
      @arow.strip!
...
class #{@name}
  def self.predict(text)
    vec = Hash.new(0)
    vec = BimyouSegmenter.segment(text)
    vec << " bias "
    if (W.size == 1)
...
MODEL
    ...
    def fv(text)
      vec = Hash.new(0)
      vec[word2id(" bias ")] = 1
      BimyouSegmenter.segment(text, :white_space => true).reject do |word|
        STOP_WORDS[word]
      end.each do |word|
        vec[word2id(word)] += 1
      end
    ...
    def normalize(vec)
      norm = Math.sqrt(vec.each_value.reduce(0){|dot, v| dot + v * v })
      if (norm > 0.0)
        s = 1.0 / norm
        vec.each do |k, v|
          vec[k] = v * s
